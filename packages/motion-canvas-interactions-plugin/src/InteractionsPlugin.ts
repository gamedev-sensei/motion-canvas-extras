...
import {InteractionSource, interactionSourceVariableKey} from "@gamedev-sensei/motion-canvas-interactions";
...
export const InteractionsPlugin = makeEditorPlugin(() => {
    ...
    function passEvent(e: Event) {
        ...
        presenterCanvas.dispatchEvent(new Event(e.type, e))
    }
    ...
    return {
        ...
        presenterOverlay: {
            drawHook: () => ctx => presenterOverlayCanvasDispatcher.dispatch(ctx.canvas)
        },
